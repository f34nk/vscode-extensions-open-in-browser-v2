...
function loadDefaultConfig() {
    return toml.parse(defaultGitProviders_1.DEFAULT_GIT_PROVIDERS_TOML);
}
describe('DynamicUrlBuilder', () => {
    let builder;
    beforeEach(() => {
        builder = new dynamicUrlBuilder_1.DynamicUrlBuilder(loadDefaultConfig());
    });
    ...
    describe('buildFileUrl', () => {
        it('builds GitHub file URLs with line number', () => {
            const url = builder.buildFileUrl(githubRemote, 'feature/test', 'src/app.ts', 10);
            assert.strictEqual(url, 'https://github.com/acme/widget/tree/feature/test/src/app.ts#L10');
        });
        it('builds GitHub file URLs with line range', () => {
            const url = builder.buildFileUrl(githubRemote, 'main', 'src/app.ts', 10, 20);
            assert.strictEqual(url, 'https://github.com/acme/widget/tree/main/src/app.ts#L10-L20');
        });
    });
    describe('buildDirectoryUrl', () => {
        it('builds GitHub directory URLs', () => {
            ...
            assert.strictEqual(url, 'https://github.com/acme/widget/tree/main/src/components');
        });
    });
    ...
    describe('buildCompareUrl', () => {
        it('builds GitHub compare URLs', () => {
            ...
            assert.strictEqual(url, 'https://github.com/acme/widget/compare/main...feature/xyz');
        });
    });
    ...
    describe('buildCommitFileUrl', () => {
        it('builds GitHub commit file URLs with line hash', () => {
            const sha = 'abc123def456789012345678901234567890abcd';
            const url = builder.buildCommitFileUrl(githubRemote, sha, 'src/app.ts', 42);
            assert.ok(url);
        ...
        it('falls back to basic commit URL when template missing', () => {
            ...
            const minimalBuilder = new dynamicUrlBuilder_1.DynamicUrlBuilder(customConfig);
            const url = minimalBuilder.buildCommitFileUrl('git@example.com:acme/app.git', 'deadbeef', 'src/main.ts', 5);
    ...
    describe('provider check order', () => {
        it('respects custom provider_check_order', () => {
            ...
            const orderedBuilder = new dynamicUrlBuilder_1.DynamicUrlBuilder(config);
...
//# sourceMappingURL=dynamicUrlBuilder.test.js.map