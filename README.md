...
Extracts Jira ticket number from the current branch name and opens it in Jira.
...
```json
{
  "open-in-browser.jiraBaseUrl": "https://your-company.atlassian.net/browse"
}
```
...
Reloads TOML configuration files without restarting VS Code.
...
```json
{
  ...
  // Jira base URL (required for "Open Jira Ticket" command)
  "open-in-browser.jiraBaseUrl": "https://your-company.atlassian.net/browse",
...
- Check branch name contains ticket number (e.g., `feature/XY-1234-description`)
- Verify `open-in-browser.jiraBaseUrl` is set correctly (without trailing slash)
